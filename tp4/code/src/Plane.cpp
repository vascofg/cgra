...
void Plane::drawCentered(){
	...
	glPushMatrix();
		...
		{
			glBegin(GL_TRIANGLE_STRIP);
				glTexCoord2f(-2+5*bx/(float)_numDivisions,0); glVertex3f(bx, 0, 0);
				for (int bz = 0; bz<_numDivisions; bz++)
				{
					
			
					glTexCoord2f(-2+5*(bx +1)/(float)_numDivisions,-1+3*(bz)/(float)_numDivisions); 
					glVertex3f(bx + 1, 0, bz);
					
					glTexCoord2f(-2+5*(bx)/(float)_numDivisions,-1+3*(bz +1)/(float)_numDivisions);
					glVertex3f(bx, 0, bz + 1);
				}
				glTexCoord2f(-2+5*(bx +1)/(float)_numDivisions,-1+3*1); glVertex3d(bx+ 1, 0, _numDivisions);
